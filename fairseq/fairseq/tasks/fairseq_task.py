...
from fairseq.data import Dictionary, FairseqDataset, data_utils, encoders, iterators, ResamplingDataset
...
class FairseqTask(object):
    ...
    def get_batch_iterator(
        ...
        Returns:
            ~fairseq.iterators.EpochBathIterator: a batched iterator over the
                given dataset split
        """
        can_reuse_epoch_itr = (
        ...
        with data_utils.numpy_seed(seed):
            if is_train and self.weights is not None:
                print("CUSTOM FAIRSEQ: Resampling.")
                mean = np.mean(list(self.weights.values()))
                weights = [
                    self.weights.get(dataset.src_dict.string(dataset.src[i]), mean)
                    for i in range(len(dataset))]
                # print(self.task.dataset(self.cfg.dataset.train_subset).src_dict.string(self.task.dataset(self.cfg.dataset.train_subset).src[-1]))
                resampled = ResamplingDataset(dataset=dataset, weights=weights, seed=seed, epoch=epoch)
                indices = np.array([resampled[i]['id'] for i in dataset.ordered_indices()])
                indices = indices[np.argsort(dataset.tgt_sizes[indices], kind="mergesort")]
                indices = indices[np.argsort(dataset.src_sizes[indices], kind="mergesort")]
            else:
    ...
    def build_model(self, cfg: FairseqDataclass, from_checkpoint=False):
        ...
        self.memorisation_weights_config_file = cfg.memorisation_weights_config_file
        if self.memorisation_weights_config_file is not None:
            print(f"CUSTOM FAIRSEQ: Loading config file {self.memorisation_weights_config_file}.")
            self.weights = pickle.load(open(self.memorisation_weights_config_file, 'rb'))
        else:
            self.weights = None
        # except:
        #     self.memorisation_weights_config_file = None
